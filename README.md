...
- Optional human-facing OS notifications are disabled by default on App, VS Code, remote, and unknown surfaces. CLI-owned jobs default to one completion notice because the open TUI cannot render the completion turn live. Enable one launch with `--notify-user`, disable it with `--no-notify-user`, or set the durable preference with `config --notify-user true|false`; an explicit flag or durable preference always overrides the CLI surface default. macOS uses `osascript`; Linux uses `notify-send` when available. These best-effort notices do not affect durable job state or conversational delivery.
