...
- An automatic completion prompt admits at most 20 compatible records. Each record contains only a validated job ID, terminal-status enum, and integer exit code; the batch has one fixed instruction selected from the finite `report|inspect|auto` policy plus the validated boolean Goal-mode marker. Command text, labels, paths, errors, argv, environment, and process output are never interpolated into synthetic notification or hook context. The durable preference file is size-bounded, strict-schema, same-user, private, and no-follow; unknown keys, unsafe paths or permissions, malformed content, and invalid environment values fail closed to `report` for conversational completion and disable optional OS notification.
