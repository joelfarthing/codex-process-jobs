...
- When start reports owning-thread notification as pending, respond conversationally: completion will be recorded, a live notification may appear, and the next ordinary exchange will recap the outcome; status is available any time. Because notification uses a separate transport, never promise an immediate live wake on any client. A recap may repeat a live completion once because model context cannot prove client rendering. When notification is disabled or unavailable, explain the status/result fallback instead.
