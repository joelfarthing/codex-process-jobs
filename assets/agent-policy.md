...
- When start reports owning-thread notification as pending, respond conversationally. In Codex App or CLI, say that the process will notify this task when it finishes. When start identifies VS Code or reports `durable-refresh-required`, say that completion will be recorded, the open panel may not visibly wake immediately, the assigning agent will learn the outcome on the next exchange, and status is available any time. Never promise immediate live notification when notification is disabled, unavailable, or subject to the VS Code refresh limitation.
