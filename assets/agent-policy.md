...
- When start reports owning-thread notification as pending, respond conversationally. In a known live Codex App or CLI surface, say that the process will notify this task when it finishes. When start identifies VS Code, `remote`, or reports `durable-refresh-required`, say that completion will be recorded, the client may not refresh the assigning agent's context immediately, the agent will learn the outcome on the next exchange, and status is available any time. Never promise immediate live notification when notification is disabled, unavailable, or subject to a refresh limitation.
