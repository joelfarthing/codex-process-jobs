...
Re-running the installer replaces that one managed block without duplicating it and preserves all unrelated personal instructions. It tells the agent to use conversational completion wording only when the controller reports notification as pending, and to disclose the open-panel refresh limitation when the controller identifies VS Code. The policy is opt-in; installation without `--with-agent-policy` never changes `AGENTS.md`.
