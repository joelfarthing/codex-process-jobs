...
5. The automatic user-facing notice is concise Markdown containing only the sanitized job id, terminal status, and exit code. It never contains the command, working directory, job label, environment, stdout, or stderr. A hidden HTML comment marks it as a plugin notification for the prompt hook.
6. The completion notice asks Codex for one short conversational acknowledgment and explicitly forbids tool calls or unrelated work.
