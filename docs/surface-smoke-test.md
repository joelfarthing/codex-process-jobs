...
```text
Use $codex-process-jobs:start to launch a harmless direct-argv Node job named surface-smoke. It should print "configure 25%", "compile 63%", and "link 100%" over about five seconds, then exit 0. Return immediately after launch, report the job id and launch time, tell me naturally that the process will notify this task when it finishes, and end the turn. Do not call status, wait, or result in the launch turn. Do not edit plugin or marketplace state.
```
...
4. Without another user prompt or a subagent, the task receives a second synthetic turn and tells the user that the job finished successfully.
