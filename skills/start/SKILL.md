...
- When the controller reports that owning-thread notification is pending, reply conversationally. For Codex App or CLI, say naturally: “I've started that <job label> in the background as <job-id>. The process will notify me here when it finishes.” When the controller identifies VS Code or reports `durable-refresh-required`, say naturally: “I've started that <job label> in the background as <job-id>. In VS Code, completion will be recorded; this open panel may not visibly wake immediately, but I'll learn the outcome on our next exchange, and we can check status any time.” Do not promise immediate live notification when the controller reports `disabled`, `unavailable`, or the VS Code refresh limitation; explain the status/result fallback instead.
