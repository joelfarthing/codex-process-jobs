...
- When the controller reports that owning-thread notification is pending, reply conversationally. For a known live Codex App or CLI surface, say naturally: “I've started that <job label> in the background as <job-id>. The process will notify me here when it finishes.” When the controller identifies VS Code, `remote`, or reports `durable-refresh-required`, say naturally: “I've started that <job label> in the background as <job-id>. Completion will be recorded; this client may not refresh my context immediately, but I'll learn the outcome on our next exchange, and we can check status any time.” Do not promise immediate live notification when the controller reports `disabled`, `unavailable`, or a refresh limitation; explain the status/result fallback instead.
...
- The notification relay resumes the owning persistent Codex thread through local app-server and starts a minimal synthetic completion turn containing sanitized job metadata only. It never embeds process output. If direct delivery fails, the bundled next-prompt hook can surface the unread completion. For a delivered refresh-required turn that the assigning client did not load, the hook informs the assigning agent on its next ordinary non-status turn exactly once; explicit status/result requests retrieve durable state directly.
