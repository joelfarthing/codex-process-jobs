...
- When the controller reports that owning-thread notification is pending, reply conversationally: “I've started that <job label> in the background as <job-id>. Completion will be recorded; a live notification may appear, and either way I'll learn the outcome by our next exchange. We can check status any time.” The completion relay uses a separate transport, so do not promise an immediate live wake on any client. When notification is `disabled` or `unavailable`, explain the status/result fallback instead.
...
- The notification relay resumes the owning persistent Codex thread through local app-server and starts a minimal synthetic completion turn containing sanitized job metadata only. It never embeds process output. On the next ordinary non-status turn, the bundled hook performs one transport-independent awareness check for every delivered completion. If the prior assistant announcement is already in context, do not repeat it; otherwise inform the user before handling the new request. Explicit status/result requests retrieve durable state directly.
