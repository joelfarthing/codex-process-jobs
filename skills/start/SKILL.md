...
- When the controller reports that owning-thread notification is pending, reply conversationally: “I've started that <job label> in the background as <job-id>. The process will notify me here when it finishes.” Do not promise notification when the controller reports `disabled` or `unavailable`; explain the status/result fallback instead.
